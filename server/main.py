...
def start_async_loop(loop, server, web_reg, item_mall_server):
    asyncio.set_event_loop(loop)
    loop.run_until_complete(run_server_stack(server, web_reg, item_mall_server))
...
def main():
    ...
    if is_headless:
        ...
        server_thread = threading.Thread(target=start_async_loop, args=(server_loop, server, web_reg, item_mall_server), daemon=True)
        server_thread.start()
